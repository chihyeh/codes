...
void jetSubstructure(string inputDir, float radius=0.4, int mode=0, float energy=0.5){
...
//     "n2_b1_mmdt"
  ...
  string inputFile = inputDir + "/radius" + Form("%0.1f",radius)+ "_rawhit_fastjet_mode" + Form("%d",mode) + "_"+Form("%.1f",energy)+"GeV.root"; //"_response_e2.root";
  ...
  for(Long64_t jEntry=0; jEntry< genTree.GetEntriesFast() ;jEntry++){
    ...
    Float_t*  tau21     = caloTree.GetPtrFloat("j_tau21_b1");
